...
def analyze_progressive_failure(raw_data: dict[str, npt.NDArray[np.float64]]) -> dict[str, typing.Any]:
    ...
    idx_start = _find_nearest_index(peak_s1, s_30)
    idx_end = _find_nearest_index(peak_s1, s_50)
    
    if idx_start >= idx_end:
        idx_end = idx_start + 1
        if idx_end >= len(peak_s1):
            idx_start = max(0, len(peak_s1) - 2)
            idx_end = len(peak_s1)

    slice_e1 = peak_e1[idx_start:idx_end]
    slice_s1 = peak_s1[idx_start:idx_end]
    slice_e3 = peak_e3[idx_start:idx_end]

    # 2. 第一级安全校验 (数据方差判定)
    if len(np.unique(slice_e1)) < 2:
        print("[WARNING] 30%-50% UCS 区间内轴向应变(e1)数据方差不足（极少有效点），触发自动安全降级，扩大寻址区间...")
        
        # 3. 第二级安全降级 (扩大容差区间 10% - 60%)
        s_10, s_60 = 0.1 * ucs_val, 0.6 * ucs_val
        idx_start = _find_nearest_index(peak_s1, s_10)
        idx_end = _find_nearest_index(peak_s1, s_60)
        if idx_start >= idx_end:
            idx_end = min(len(peak_s1), idx_start + 2)
        ...
        # 4. 极端异常处理
        if len(np.unique(slice_e1)) < 2:
            raise ValueError("ZDEM 记录频率过低，无法捕捉到有效的弹性阶段应变方差，请减小 ZDEM 历史记录的 Interval！")
